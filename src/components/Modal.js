...
const Modal = ({ loginModal, loginModalHandler, backdropHandler }) => {
    ...
    if (email !== "" && password !== "") {
      signInWithEmailAndPassword(auth, email, password)
        ...
        .catch((error) => {
          ...
          setSpinner(false);
          toast.error(
            "unable to register please confirm authentication credentials",
            {
              position: "top-right",
              autoClose: 5000,
              hideProgressBar: false,
  ...
  const resetPassword = (e) => {
    ...
    if (email === "") {
      toast.error(
        "please input your email in the email field then click on reset password",
        {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: false,
      ...
      {loginModal && (
        ...
        >
          <form className="bg-white w-72 lg:w-96 mx-auto mt-44 p-5 rounded-md relative">
            ...
            <FontAwesomeIcon
              icon={faClose}
              className="mr-2 text-gray-500 font-bold pt-1 md:pt-0 text-xl lg:text-2xl cursor-pointer absolute top-0 right-0 mt-2"
            />
            <div className="my-3 ">
            ...
            <div>
              <button
                className="text-white bg-indigo-800 w-full py-2 rounded-md text-lg text-center"
            ...
            <div className="my-3">
              <button
                className="border-indigo-800 border-2 text-indigo-800 w-full text-lg py-2 rounded-md"
