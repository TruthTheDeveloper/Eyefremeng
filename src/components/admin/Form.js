...
const Form = ({ formText, product, price }) => {
  ...
  const updateProduct = async () => {
    ...
    try {
      ...
      setSpinner(false);
      toast.error(
        "Unable to Update Product, contact developer for possible fix",
        {
          position: "top-right",
          autoClose: 5000,
          hideProgressBar: false,
    ...
    <>
      <form className="xl:w-4/12 lg:w-5/12 md:w-6/12  mx-auto p-3 border border-slate-400 rounded-md">
        <div className=" my-4 ">
          ...
          <input
            className="h-8 w-full mt-1 border border-slate-400 outline-none p-2"
        ...
        <div>
          ...
          <input
            type="text"
            className="h-8 w-full mt-1 border border-slate-400 outline-none p-2"
            onChange={(e) => setTempleWidth(e.target.value)}
            value={templeWidth}
          />
        </div>
        <div className=" my-4">
          ...
          <input
            type="file"
            className="mt-1"
            onChange={(e) => setFrontView(e.target.files[0])}
          />
        </div>
        <div className=" my-4">
          ...
          <input
            type="file"
            className="mt-1"
            onChange={(e) => setLeftView(e.target.files[0])}
          />
        </div>
        <div className=" my-4">
          ...
          <input
            type="file"
            className="mt-1"
            onChange={(e) => setRightView(e.target.files[0])}
          />
        </div>
        <button
          className=" my-6 mx-auto flex px-8 py-2 rounded-md bg-indigo-800 text-white"
          onClick={addProductSubmitHandler}
        >
          {spinner ? (
            <div className="w-full ">
              <TailSpin color="white" height={30} width={220} />
            </div>
          ) : (
            formText
          )}
      ...
      <button
        className="py-2 px-12 rounded-md border border-slate-700  mx-auto flex my-5"
        onClick={() => navigate(-1)}
      >
        Back
      </button>
