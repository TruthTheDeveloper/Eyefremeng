...
const BilingForm = () => {
  ...
  const shipment = (e) => {
    ...
    firstName === ""
      ? setFirstNameValidationError("Please input first Name")
      : setFirstNameValidationError("");
    lastName === ""
      ? setLastNameValidationError("Please input last name")
      : setLastNameValidationError("");
    address === ""
      ? setAddressValidationError("Please input address")
      : setAddressValidationError("");
    city === ""
      ? setCityValidationError("Please input city")
      : setCityValidationError("");
    state === ""
      ? setStateValidationError("Please input state/province")
      : setStateValidationError("");
    postalCode === ""
      ? setPostalCodeValidationError("Please input postal Code")
      : setPostalCodeValidationError("");
    telephone === null
      ? setTelephoneValiadtionError("Please input teelphone address")
      : setTelephoneValiadtionError("");
    email === null
      ? setEmailValidationError("Please input Email")
      ...
      <div className="border bg-slate-50 my-12 ">
        <div className="md:flex md:my-4">
          <div className="px-4 my-4 md:my-0 md:w-full">
            ...
            <br />
            <input
              className="w-full my-2 h-10 p-2 outline-none border-2 border-slate-300 rounded-sm"
              value={firstName}
              onChange={(e) => setFirstName(e.target.value)}
            />
            {firstNameValidationError && (
              <p className="text-red-500 text-sm">{firstNameValidationError}</p>
            )}
          </div>
          <div className="md:w-full px-4">
            ...
            {lastNameValidationError && (
              <p className="text-red-500 text-sm">{lastNameValidationError}</p>
            )}
          </div>
        </div>
        <div className="px-4 ">
          ...
          {addressValidationError && (
            <p className="text-red-500 text-sm">{addressValidationError}</p>
          )}
        </div>
        <div className="px-4 ">
          <label>Email</label>
        ...
        <div className="md:flex">
          <div className="px-4 w-full">
            ...
            />
            {cityValidationError && (
              <p className="text-red-500 text-sm">{cityValidationError}</p>
            )}
          </div>
          <div className="px-4 w-full">
            ...
            {stateValidationError && (
              <p className="text-red-500 text-sm">{stateValidationError}</p>
            )}
          </div>
          <div className="px-4 w-full">
        ...
        <div className="px-4 py-8">
          ...
          {telephoneValidationError && (
            <p className="text-red-500 text-sm">{telephoneValidationError}</p>
          )}
        </div>
      </div>
      <div>
